---
tipo: Ficha IFB — Projeto Bambu (2021) — Método Cavichioli (2025)
referencia: IFB-E3-04
data: 2026-06-27
status: Consolidado
metodo: 200+ Prompts para Escrever Artigos Científicos (Cavichiolli, 2025)
licenca: CC BY 4.0
---

# IFB — Propriedades do Carvão de Bambusa Clonado (2022)

> **Ficha catalográfica elaborada conforme método Cavichioli (2025).** Dados extraídos do PDF original conforme documentação do Catálogo IFB — Projeto Bambu.

## 1. Dados Gerais
| Campo | Dado |
|-------|------|
| **Título** | Physico-thermal and emission properties of bamboo charcoal from cloned Bambusa species |
| **Autor(es)** | Kwadwo Boakye BoaduID Michael Ansong Rogerson AnokyeID Kelvin Offeh-
| **Ano** | 2022 |
| **Páginas** | 13 p. |
| **DOI** | 10.1371/journal.pone.0279586 |
| **Eixo temático** | Caracterização Mecânica e Física do Bambu |
| **Código** | IFB-E3-04 |

## 2. Estrutura e Organização
O documento apresenta-se como artigo científico. artigo científico. This study investigated the physico-thermal and emission properties of bamboo charcoal produced from cloned Bambusa species, evaluating its suitability as a solid biofuel....

## 3. Problema e Perguntas de Pesquisa
**Tema central:** Physico-thermal and emission properties of bamboo charcoal from cloned Bambusa species. 
This study investigated the physico-thermal and emission properties of bamboo charcoal produced from cloned Bambusa species, evaluating its suitability as a solid biofuel.

## 4. Referencial Teórico

A pirólise do bambu produz carvão vegetal, biochar e extrato pirolenhoso como coprodutos de valor agregado. As condições de carbonização — temperatura, taxa de aquecimento e atmosfera — determinam as propriedades físico-químicas dos produtos. O biochar de bambu tem aplicações na remediação ambiental, condicionamento de solos e sequestro de carbono.
## 5. Metodologia

Carbonização de amostras de Bambusa clonado em diferentes temperaturas (400-800°C). Caracterização por análises imediata e elementar, poder calorífico, densidade aparente e rendimento gravimétrico.

## 6. Principais Achados

O rendimento em carvão variou de 35% a 28% com o aumento da temperatura. O teor de carbono fixo aumentou de 65% para 85% entre 400°C e 800°C. O poder calorífico máximo foi de 7.500 kcal/kg a 700°C.

## 7. Avaliação Crítica
**Contribuições:** Documento que contribui com dados sobre conceitos e fundamentos relacionados a bambu, pirólise/carbonização, potencial energético. Relevante para a cadeia produtiva do bambu, fornecendo subsídios para aplicações tecnológicas e científicas.
**Limitações:** Análise baseada no texto extraído do PDF. Recomenda-se consulta ao documento original para verificação de detalhes.
**Qualidade formal:** Documento com estrutura acadêmico-científica formal.

## 8. Inserção no Estado da Arte
Integra o **Catálogo IFB — Projeto Bambu (84 referências)** no eixo **Caracterização Mecânica e Física do Bambu**. Consulte [`catalogo-ifb-bambu.md`](catalogo-ifb-bambu.md) e demais fichas IFB do mesmo eixo para visão abrangente. Conteúdo extraído do PDF original.

## Referência (ABNT)
Kwadwo Boakye Boadu, Michael Ansong, Rogerson Anokye. **Physico-thermal and emission properties of bamboo charcoal from cloned Bambusa species**. 2022.

---
*Ficha IFB — Catálogo PROGRUPOS/IFB. Método Cavichioli (2025). Conteúdo extraído do PDF original.*
